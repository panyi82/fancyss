...
create_dnsmasq_conf() {
	...
	if [ "${ss_basic_advdns}" != "1" ]; then
		if [ "${ss_basic_mode}" == "6" ]; then
			...
			cat /koolshare/ss/rules/cdn.txt | sed "s/\(apple.com\|microsoft.com\|google.com\|okii.com\)/d/g" | sed "s/^/server=&\/./g" | sed "s/$/\/&127.0.0.1#$DNSF_PORT/g" | sort | awk '{if ($0!=line) print;line=$0}' >>/tmp/cdn.conf
		else
